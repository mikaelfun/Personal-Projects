...
class MyMineSweeper:
    def __init__(self):
        ...
        self.myfont2 = pygame.font.SysFont('cambria', 16)
    ...
    def drawOption(self):
        ...
        sizeSpecifics1 = self.myfont2.render("10-50", False, (0, 0, 0))
        self.surface.blit(sizeSpecifics1, (self.optionGridPos[0] + 134, self.optionGridPos[1] + 66))
        # self.surface.blit(self.image["input"], (self.optionGridPos[0] + 124, self.optionGridPos[1]))

        self.surface.blit(self.image["input"], (self.optionGridPos[0] + 252, self.optionGridPos[1]))
        sizeSpecifics2 = self.myfont2.render("6-24", False, (0, 0, 0))
        self.surface.blit(sizeSpecifics2, (self.optionGridPos[0] + 262, self.optionGridPos[1] + 66))
        # Xmark
        ...
        sizeSpecifics3 = self.myfont2.render("1-99", False, (0, 0, 0))
        self.surface.blit(sizeSpecifics3, (self.optionBombCountPos[0] + 134, self.optionBombCountPos[1] + 66))
        for box in self.input_boxes:
            box.update()
        for box in self.input_boxes:
            box.draw(self.surface)

    def optionOnclickYes(self):
        if self.resize_box1.text == "" or self.resize_box2.text == "" or self.resize_box3.text == "":
            print ("Invalid Input")
            return None
        if (int(self.resize_box1.text) > 50 or \
                    int(self.resize_box1.text) < 10 or \
                    int(self.resize_box2.text) < 6 or \
                    int(self.resize_box2.text) > 24) or \
                    int(self.resize_box3.text) > 99 or \
                    int(self.resize_box3.text) == 0:
            return None
        if int(self.resize_box3.text) > int(self.resize_box1.text) * int(self.resize_box2.text):
            return None
        self.prevState = self.gameState
        self.gameState = GameState.waiting
    ...
    def optionOnclickNo(self):
        self.resize_box1.text = str(self.width)
        self.resize_box2.text = str(self.height)
        self.resize_box3.text = str(self.mine)
