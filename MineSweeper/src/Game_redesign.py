...
class MineSweeper:
    ...
    def run(self):
        ...
        resize_box1 = InputBox(self.boarderWidth + (self.x/2 - 2) * self.tileSize , self.windowHeight / 2 + self.boarderHeight, 50, 50, str(self.x))
        resize_box2 = InputBox(self.boarderWidth + (self.x/2 ) * self.tileSize, self.windowHeight / 2+ self.boarderHeight , 50, 50,  str(self.y))
        resize_box3 = InputBox(self.boarderWidth + (self.x/2 + 2) * self.tileSize, self.windowHeight / 2+ self.boarderHeight, 50, 50, str(self.n))
        xMark = InputBox(self.boarderWidth + (self.x/2 - 1) * self.tileSize , self.windowHeight / 2+ self.boarderHeight, 50, 50, "X", False)


        input_boxes = [resize_box1, resize_box2, resize_box3, xMark]
        goMark = pygame.sprite.Sprite()
        goMark.image = self.image['GO']
        goMark.rect = goMark.image.get_rect()
        goMark.rect.x, goMark.rect.y = (self.boarderWidth + (self.x/2 + 3) * self.tileSize , self.windowHeight + self.boarderHeight - self.tileSize)
        self.all_sprites.add(goMark)
        ...
        while True: # main game loop
            ...
            for event in pygame.event.get():
                ...
                if event.type == QUIT:
                    ...
                    if (r.colliderect(goMark.rect)):
                        if (int(resize_box1.text) > 50 or int(resize_box1.text) < 10 or int(resize_box2.text) < 5 or int(resize_box3.text) > int(resize_box1.text) * int(resize_box2.text) or int(resize_box2.text) > 24):
                            print ("Invalid Input")
                            break
                        self.x = int(resize_box1.text)
                        self.y = int(resize_box2.text)
                        self.n = int(resize_box3.text)
                        self.updateTileSize()
            ...
            if timer > 999:
                timer = 999
            timerDis = myfont1.render(str(round(timer, 2)), False, (0, 0, 0))
            self.surface.blit(timerDis,(self.screenWidth - self.boarderWidth - 100, 10))

            # display Bombs left
            BombLeftDis = myfont1.render("Bombs left: "+str(bombLeft), False, (0, 0, 0))
