...
class MyMineSweeper:
    def __init__(self):
        ...
        self.optionWindowW = 640
        self.optionWindowH = 320
        self.optionYESPos = (self.windowW - (self.windowW - self.optionWindowW)/2 - 158, self.windowH - (self.windowH - self.optionWindowH)/2 - 74)
        self.optionNOPos = (self.windowW - (self.windowW - self.optionWindowW)/2 - 74, self.windowH - (self.windowH - self.optionWindowH)/2 - 74)
    ...
    def image_init(self):
        ...
        self.image["optionBig"] = pygame.transform.scale(self.image["optionBig"], (68, 68))
        ...
        self.image["normalBig"] = pygame.transform.scale(self.image["normalBig"], (68, 68))
        ...
        self.image["winBig"] = pygame.transform.scale(self.image["winBig"], (68, 68))
        self.image["loseBig"] = pygame.image.load("Img\\lose.png")
        self.image["loseBig"] = pygame.transform.scale(self.image["loseBig"], (68, 68))
    ...
    def adjustTileSize(self):
        if self.windowW > 1920 and self.windowH < 1000:
            # only shrink width
            if 1920 - self.tileSize * self.width >= 100:
                # boardwidth >=50 is acceptable
                self.boarderWidth = (1920 - self.tileSize * self.width) // 2
                self.windowW = self.tileSize * self.width + self.boarderWidth*2
                # print (".1")
            else:
                self.boarderWidth = 50
                self.windowW = 1920
        ...
        elif self.windowW <= 1920 and self.windowH > 1000:
            # only shrink height
            self.windowH = 1000
    ...
    def update_window(self):
        ...
        self.windowW = (self.width * self.tileSize + 2 * self.boarderWidth)
        self.windowH = (self.height * self.tileSize + self.boarderHeight + self.controlboarderHeight)
        if self.windowW > 1920 or self.windowH > 1000:
            self.adjustTileSize()
            self.image_init()
        else:
    ...
    def draw_background(self):
        ...
        # self.windowW - self.boarderWidth - 64
                ...
                if curPos == (-1, -1):
                    ...
                    if self.leftmouseDownPos == (-2, -2) and self.leftmouseDownPos == self.getMouseIndexOnArray(pygame.mouse.get_pos()):
                        ...
                        if self.leftmouseDownPos[0] >= 0:
                            ...
                            if self.gameState == GameState.running:
                                self.smile.image = self.image["normal"]
                                if self.tileArray[self.leftmouseDownPos[0]][self.leftmouseDownPos[1]].state == TileState.hidden or \
                                                self.tileArray[self.leftmouseDownPos[0]][self.leftmouseDownPos[1]].state == TileState.marked:
                                    self.update_tile(self.leftmouseDownPos[0], self.leftmouseDownPos[1])
                                if self.gameState == GameState.lose:
                                    self.smile.image = self.image["lose"]
                                    self.game_over()
                self.leftMouseDown = False
